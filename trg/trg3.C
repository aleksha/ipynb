...
void trg3(){
  ...
  for(int r=0;r<14;r++){
    ...
    for(int tev=0;tev<tree->GetEntries();tev++){
      tree->GetEntry( tev );
      if( fabs(X)<vR[r]*0.5 && fabs(Y)<vR[r]*0.5 ) pRate[r] = pRate[r]+1.;
    }
    ...
    for(int tev=0;tev<free->GetEntries();tev++){
      free->GetEntry( tev );
      if( fabs(fX)<vR[r]*0.5 && fabs(fY)<vR[r]*0.5 ) fRate[r] = fRate[r]+1.;
    }
  ...
  gf->GetXaxis()->SetTitle("Side length, mm");
