...
for n in range(1,26):
    means = []
    sums  = []
    for i in progress_bar( range(1,1001) ):
        fff  = "/home/adzyuba//data/akip/20210511-"
        fff += num2str(n) + "/20210511-" + num2str(n) + "_"
        fff += num2str(i) + ".txt"
        h_in, h_out, hq = load_event( fff , num2str(n)+"_"+num2str(i) )
        gs, hs = get_signal( hq )
        means.append( hs.GetMean() )
        sums .append( hs.sum().value() )
    print( "Time   :" + str( mean( means ) ) + "  +/- " + str( stdev( means) ) )
    print( "Energy :" + str( mean( sums  ) ) + "  +/- " + str( stdev( sums ) ) )
    mm.append( means )
    ss.append( sums  )
